...
export async function fetchEDTData(dateFilter: ApiFilter<number>, modules: ModuleChoice[]): Promise<CourseEvent[]> {
    if (modules == undefined || modules.length <= 0) return [];

    const payload = {dateFilter: dateFilter, modules: modules.map(m => m.code)};
